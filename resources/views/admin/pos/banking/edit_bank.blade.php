...
<section class="content">
      <div class="container-fluid">
      <form action="{{url('/dashboard/edit_bank/'.$id)}}" id="addProduct" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row">
                  ...
                  <div class="tab-pane fade active show add_card" id="custom-tabs-four-home" role="tabpanel" aria-labelledby="custom-tabs-four-home-tab">
                        <div class="form-group">
                            ...
                            <select id="category" name="category" class="form-control custom-select">
                                <option value="general_bank" {{$bank->type=="general_bank" ? 'selected' : '' }} >General Bank</option>
                                <option value="mobile_bank" {{$bank->type=="mobile_bank" ? 'selected' : '' }}>Mobile Bank</option> 
                            </select>
                        </div>
                        
                        <div class="form-group">
                            <label for="bank_name">Bank Name<span class="text-danger">*</span></label>
                            <input type="text" name="bank_name" id="bank_name" class="form-control" value="{{$bank->name}}">
                        </div> 
                        <div class="form-group" id="bank_address_div">
                            <label for="bank_address">Bank Address<span class="text-danger">*</span></label>
                            <input type="text" name="bank_address" id="bank_address" class="form-control" value="{{$bank->address ?? ''}}">
                        </div> 
                        <div class="form-group" id="acc_name_div">
                            <label for="acc_name">Bank Account Name<span class="text-danger">*</span></label>
                            <input type="text" name="acc_name" id="acc_name" class="form-control" value="{{$bank->account->acc_name ?? ''}}">
                        </div>
                        <div class="form-group">
                            <label for="acc_no">Bank Account No<span class="text-danger">*</span></label>
                            <input type="text" name="acc_no" id="acc_no" class="form-control" value="{{$bank->account->acc_no ?? ''}}">
                        </div>
                        <div class="form-group">
                            <div>
                        ...
                        @if($cards)
                            @foreach($cards as $card)
                                <div class="form-group cards_name">
                                    ...
                                    <input type="text" name="card_name_prev[{{$card->id}}]" id="card_name_prev" class="form-control" value="{{$card->card_name ?? ''}}">
                                </div>
                            @endforeach
                        @endif
                  </div>
        ...
        <div class="row pb-5">
            <div class="col-12">
                ...
                <input type="submit" value="Update Bank" class="btn btn-success float-right">
            </div>
        </div>
        </form>
      </div>
